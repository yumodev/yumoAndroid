...
//                    "requested destination file already exists");
            ...
            return createNotRepeatFileName(path);
        }
    ...
    private static String createNotRepeatFileName(String path){
        String fileName = Uri.parse(path).getLastPathSegment();
        int lastPotIndex = fileName.lastIndexOf(".");
        String extension = "";
        if (lastPotIndex != -1){
            extension = fileName.substring(lastPotIndex);
        }

        String absPath = (new File(path)).getParentFile().getAbsolutePath();
        int i = 1;
        while (true){
            String newPath = absPath + File.separator + fileName+"-"+i+extension;
